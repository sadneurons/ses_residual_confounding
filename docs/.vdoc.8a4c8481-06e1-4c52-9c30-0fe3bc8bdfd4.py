...
output = display(Markdown(f"""**Background.** Socioeconomic status (SES) is often modeled linearly or as coarse categories in large cohorts. When true SES&rarr;risk relationships are non-linear and samples are selected (healthy volunteer bias), SES's role can be under-estimated.
